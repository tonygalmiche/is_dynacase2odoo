...
class IsFNC(models.Model):
    ...
    # Coûts / Bilan
    # ------------------------------------------------------------
    total_avoir = fields.Float(
        string="Total des avoirs",
        digits=(16, 2),
        tracking=True,
        compute="_compute_total_avoir",
        store=False,
        readonly=True,
    )

    @api.depends('avoir_ids.montant_avoir')
    def _compute_total_avoir(self):
        for rec in self:
            rec.total_avoir = sum(line.montant_avoir or 0.0 for line in rec.avoir_ids)

    date_bilan = fields.Date(string="Date Bilan", tracking=True)
    situation_bilan = fields.Char(string="Décision bilan", tracking=True)

    cout_piece = fields.Float(
        string="Total coûts pièces",
        digits=(16, 2),
        tracking=True,
        help="Total coûts des produits concernés (Prix de vente x Quantité détruite)",
        compute="_compute_cout_piece",
        store=False,
        readonly=True,
    )

    @api.depends('produit_ids.prix_vente', 'produit_ids.qt_detruite')
    def _compute_cout_piece(self):
        for rec in self:
            total = 0.0
            for line in rec.produit_ids:
                total += (line.prix_vente or 0.0) * (line.qt_detruite or 0.0)
            rec.cout_piece = total




    cout_total_tri = fields.Float(
        string="Total coûts tri",
        digits=(16, 2),
        tracking=True,
        help="Total de 'Nbre de pcs non-conformes' x 'Coût du tri'",
        compute="_compute_cout_total_tri",
        store=False,
        readonly=True,
    )

    @api.depends('action_ids.nb_non_conforme', 'action_ids.cout_tri')
    def _compute_cout_total_tri(self):
        for rec in self:
            total = 0.0
    ...
    total_autrecouts = fields.Float(
        string="Total autres coûts non-qualité",
        digits=(16, 2),
        tracking=True,
        help="Total Autres coûts non qualité",
        compute="_compute_total_autrecouts",
        store=False,
        readonly=True,
    )

    @api.depends('autre_cout_ids.montant')
    def _compute_total_autrecouts(self):
        for rec in self:
            rec.total_autrecouts = sum(line.montant or 0.0 for line in rec.autre_cout_ids)

    cout_total = fields.Float(
        string="Coût TOTAL",
        digits=(16, 2),
        tracking=True,
        help="Total des coûts",
        compute="_compute_cout_total",
        store=False,
        readonly=True,
    )

    @api.depends('cout_piece', 'cout_total_tri', 'total_autrecouts', 'cout_facture')
    def _compute_cout_total(self):
        for rec in self:
            rec.cout_total = (
                (rec.cout_piece or 0.0)
                + (rec.cout_total_tri or 0.0)
                + (rec.total_autrecouts or 0.0)
                + (rec.cout_facture or 0.0)
            )

    total_perte = fields.Float(
        string="TOTAL coût non-qualité",
        digits=(16, 2),
        tracking=True,
        help="TOTAL coût non-qualité",
        compute="_compute_total_perte",
        store=False,
        readonly=True,
    )

    @api.depends('cout_piece', 'cout_total_tri', 'total_autrecouts', 'cout_facture')
    def _compute_total_perte(self):
        for rec in self:
            rec.total_perte = (
                (rec.cout_piece or 0.0)
                + (rec.cout_total_tri or 0.0)
        ...
        for obj in self:
            ...
            for contact in obj.client_id.child_ids:
                ...
                if found:
                    break
            if res:
                name = "%s <%s>"%(res.name, res.email)
                obj.contact_client = name

            if obj.client_id.user_id.email:
                obj.mail_commercial = obj.client_id.user_id.email
