...
class is_plan_amelioration_continu(models.Model):
    ...
    annee               = fields.Char('Année', tracking=True)
    mois                = fields.Char('Mois', tracking=True)
    plan_action_id      = fields.Many2many('is.plan.action','is_plan_amelioration_continu_plan_action_rel','plan_amelioration_continu_id','plan_action_id', string="Plan d'actions", tracking=True)
