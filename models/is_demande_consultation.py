...
class IsDemandeConsultation(models.Model):
    ...
    @api.depends('line_ids', 'line_ids.fournisseur_ids.date_envoi_mail', 'state', 'type_consultation', 
                 'demandeur_id', 'validateur_technique_id', 'acheteur_id', 'demande_line_ids')
    def _compute_vsb(self):
        """Calcule la visibilité des boutons de workflow"""
        uid = self._uid
        for obj in self:
            # Bouton vers brouillon (bloqué depuis consultation_en_cours, solde, annule, et si lignes générées)
            vsb = False
            if not obj.line_ids:
            ...
            # Bouton saisie rapide des prix
            vsb = False
            if obj.state in ['transmis_achat', 'consultation_en_cours'] and obj.line_ids:
                vsb = True
            obj.saisie_rapide_prix_vsb = vsb
    ...
    # Lignes
    line_ids = fields.One2many('is.demande.consultation.line', 'demande_id', string="Lignes", copy=False)
    ...
    generer_lignes_vsb = fields.Boolean('Champ technique generer_lignes_vsb', compute='_compute_vsb', store=False)
    saisie_rapide_prix_vsb = fields.Boolean('Champ technique saisie_rapide_prix_vsb', compute='_compute_vsb', store=False)
    lignes_generees = fields.Boolean('Lignes générées', compute='_compute_lignes_generees', store=False)

    @api.depends('line_ids', 'demande_line_ids')
    def _compute_lignes_generees(self):
        for obj in self:
            obj.lignes_generees = bool(obj.demande_line_ids and obj.line_ids)
    ...
    def _onchange_dossier_ao_id(self):
        ...
        for obj in self:
            ...
            if dao:
                ...
                mold = self.env['is.mold'].search([('dossier_appel_offre_id', '=', dao.id)], limit=1)
                if mold:
                    obj.mold_id = mold
                # Chercher le dossier F lié à ce dossier AO
                dossierf = self.env['is.dossierf'].search([('dossier_appel_offre_id', '=', dao.id)], limit=1)
                if dossierf:
    ...
    def vers_brouillon_action(self):
        ...
        for obj in self:
            if obj.line_ids:
                raise ValidationError("Impossible de revenir en brouillon : des lignes ont déjà été générées.")
    ...
    def _get_email_commercial(self, partner):
        """Retourne l'email commercial d'un fournisseur : contact avec type 'Commercial', sinon email principal."""
        for contact in partner.child_ids:
            if contact.is_type_contact and contact.is_type_contact.name and \
               'commercial' in contact.is_type_contact.name.lower() and contact.email:
    ...
    def generer_lignes_action(self):
        ...
        for obj in self:
            ...
            if obj.line_ids:
                raise ValidationError("Les lignes ont déjà été générées.")
            # Vérifier que chaque ligne a au moins un fournisseur et que tous ont un email
            erreurs_fournisseur = []
            erreurs_email = []
            for dl in obj.demande_line_ids:
                if obj.type_consultation in ['dc_comp', 'dc_emb']:
                    label_ligne = dl.type_produit or f"Ligne {dl.sequence or ''}"
                ...
                autres = [l.strip() for l in (dl.autres_fournisseurs or '').splitlines() if l.strip()]
                if not dl.fournisseur_ids and not autres:
                    erreurs_fournisseur.append(f"Ligne '{label_ligne}' : aucun fournisseur renseigné")
                    continue
                for partner in dl.fournisseur_ids:
                    if not self._get_email_commercial(partner):
                        erreurs_email.append(f"Ligne '{label_ligne}' - Fournisseur panel : {partner.name}")
                mails = [l.strip() for l in (dl.mails_autres_fournisseurs or '').splitlines()]
                for i, autre in enumerate(autres):
                    mail = mails[i] if i < len(mails) else ''
                    if not mail:
                        erreurs_email.append(f"Ligne '{label_ligne}' - Autre fournisseur : {autre}")
            if erreurs_fournisseur:
                raise ValidationError(
                    "Impossible de générer les lignes : aucun fournisseur pour :\n- " + "\n- ".join(erreurs_fournisseur)
                )
            if erreurs_email:
                raise ValidationError(
                    "Impossible de générer les lignes : email manquant pour :\n- " + "\n- ".join(erreurs_email)
                )
            # Générer les lignes
            obj.line_ids.sudo().unlink()
            seq = 1
            for dl in obj.demande_line_ids:
                lots = [l.strip() for l in (dl.lot or '').splitlines() if l.strip()]
                if not lots:
                    lots = ['']
                for lot_val in lots:
                    fournisseur_vals = []
                    # Fournisseurs panel
                    for partner in dl.fournisseur_ids:
                        fournisseur_vals.append((0, 0, {
                            'fournisseur_id': partner.id,
                            'email_commercial': self._get_email_commercial(partner),
                        }))
                    # Autres fournisseurs hors panel
                    autres = [l.strip() for l in (dl.autres_fournisseurs or '').splitlines() if l.strip()]
                    mails = [l.strip() for l in (dl.mails_autres_fournisseurs or '').splitlines()]
                    for i, autre in enumerate(autres):
                        mail = mails[i] if i < len(mails) else ''
                        fournisseur_vals.append((0, 0, {
                            'fournisseur_hors_panel': autre,
                            'email_commercial': mail,
                        }))
                    if obj.type_consultation == 'dc_comp':
                        line_vals = {
                            'demande_id': obj.id,
                            'sequence': seq,
                            'type_produit': dl.type_produit,
                            'matiere': dl.matiere,
                    ...
                    elif obj.type_consultation == 'dc_emb':
                        line_vals = {
                            'demande_id': obj.id,
                            'sequence': seq,
                            'type_produit': dl.type_produit,
                            'matiere': dl.matiere,
                            'dimensions_int': dl.dimensions_int,
                            'dimensions_ext': dl.dimensions_ext,
                        ...
                        line_vals = {
                            ...
                            'poids': dl.poids,
                            'dimensions_moule': dl.dimensions_moule,
                    ...
                    self.env['is.demande.consultation.line'].sudo().create(line_vals)
                    seq += 1

    def saisie_rapide_prix_action(self):
        """Ouvre une vue liste des lignes fournisseurs pour saisie rapide des prix"""
        for obj in self:
            fournisseur_line_ids = self.env['is.demande.consultation.line.fournisseur'].search(
                [('line_id', 'in', obj.line_ids.ids)]
            ).ids
            # Sélectionner la vue adaptée au type de consultation
            view_xmlid = {
                'dc_mat': 'is_dynacase2odoo.is_demande_consultation_line_fournisseur_list_view_dc_mat',
                'dc_comp': 'is_dynacase2odoo.is_demande_consultation_line_fournisseur_list_view_dc_comp',
            ...
            return {
                'name': f'Saisie rapide des prix - {obj.name}',
                'type': 'ir.actions.act_window',
                'res_model': 'is.demande.consultation.line.fournisseur',
                'view_mode': 'list',
                'views': [(view_id, 'list')] if view_id else [(False, 'list')],
                'domain': [('id', 'in', fournisseur_line_ids)],
    ...
    def envoi_consultation_fournisseurs_action(self):
        ...
        for obj in self:
            ...
            # Vérifier que les lignes ont bien été générées (dc_mat)
            if obj.type_consultation == 'dc_mat' and obj.demande_line_ids and not obj.line_ids:
                raise ValidationError("Veuillez d'abord générer les lignes via le bouton 'Générer les lignes'.")
...
class IsDemandeConsultationDemandeLine(models.Model):
    _name = 'is.demande.consultation.demande.line'
    _description = "Saisie commerciale - Demande de consultation"
    ...
    def _check_lot(self):
        for obj in self:
            if obj.lot:
                for ligne in obj.lot.splitlines():
                    ligne = ligne.strip()
                    if ligne and not ligne.isdigit():
                        raise ValidationError(
                            f"Le champ 'Lot' ne doit contenir que des nombres entiers (un par ligne). "
                            f"Valeur incorrecte : '{ligne}'"
                        )
    ...
    matiere = fields.Char("Matière", help="acier, plastique, carton PE...")
    ...
    poids_emballage = fields.Float("Poids dans l'emballage", digits=(14, 4))
    # Champs spécifiques DC-PORT
    colisage = fields.Char("Colisage")
    poids = fields.Float("Poids", digits=(14, 2))
    frequence = fields.Char("Fréquence")
    # Champs spécifiques DC-CHINE
...
class IsDemandeConsultationLineFournisseur(models.Model):
    ...
    type_consultation = fields.Selection(related='line_id.type_consultation', string="Type de consultation", store=True)
    sequence = fields.Integer('Ordre')
    ...
    dimension = fields.Char(related='line_id.dimension', string="Dimension", store=False, readonly=True)
    environnement = fields.Char(related='line_id.environnement', string="Environnement", store=False, readonly=True)
    dimensions_int = fields.Char(related='line_id.dimensions_int', string="Dimensions intérieures", store=False, readonly=True)
    dimensions_ext = fields.Char(related='line_id.dimensions_ext', string="Dimensions extérieures", store=False, readonly=True)
    type_cannelure = fields.Char(related='line_id.type_cannelure', string="Type de cannelure", store=False, readonly=True)
    poids_emballage = fields.Float(related='line_id.poids_emballage', string="Poids dans l'emballage", store=False, readonly=True)
    standard = fields.Selection(related='line_id.standard', string="Standard", store=False, readonly=True)
    ...
    dimensions_moule = fields.Char(related='line_id.dimensions_moule', string="Dimensions du moule", store=False, readonly=True)
    date_disponibilite_outillage = fields.Date(related='line_id.date_disponibilite_outillage', string="Date de disponibilité de l'outillage", store=False, readonly=True)
    date_livraison_souhaitee = fields.Date(related='line_id.date_livraison_souhaitee', string="Date de livraison maxi souhaitée", store=False, readonly=True)
    mode_transport = fields.Selection(related='line_id.mode_transport', string="Mode de transport", store=False, readonly=True)
