...
class is_inv_achat_moule(models.Model):
    ...
    nature             = fields.Char(string="Nature DM / Désignation ERD"                        , tracking=True, compute='_compute',store=False, readonly=True)
    clientid           = fields.Many2one("res.partner"              , string="Client"            , tracking=True, compute='_compute',store=False, readonly=True)
    projetid           = fields.Many2one("is.mold.project"          , string="Projet"            , tracking=True, compute='_compute',store=False, readonly=True)
    chef_projetid      = fields.Many2one("res.users"                , string="Chef de Projet"    , tracking=True, compute='_compute',store=False, readonly=True)
    date_saisie        = fields.Date(string="Date de saisie"                                     , tracking=True)
    annee_saisie       = fields.Char(string="Année de saisie"                                    , tracking=True, compute='_compute_annee',store=False, readonly=True)
    ...
    num_cde            = fields.Char(string="N°Cde fournisseur"                                  , tracking=True, compute='_compute_cde',store=False, readonly=True)
    fournisseurid      = fields.Many2one("res.partner", string="Fournisseur"                     , tracking=True, compute='_compute_cde',store=False, readonly=True)
    code_fournisseur   = fields.Char(string="Code fournisseur"                                   , tracking=True, compute='_compute_cde',store=False, readonly=True)
    date_cde           = fields.Date(string="Date commande"                                      , tracking=True, compute='_compute_cde',store=False, readonly=True)
    objet_commande     = fields.Text(string="Objet de la commande"                               , tracking=True, compute='_compute_cde',store=False, readonly=True)
    prix_commande      = fields.Float(string="Montant de la commande fournisseur"                , tracking=True, compute='_compute_cde',store=False, readonly=True)
