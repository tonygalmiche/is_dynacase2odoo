# -*- coding: utf-8 -*-
{
    "name"     : "Module Odoo 16 pour Plastigray pour migrer les développements de Dynacase dans Odoo",
    "version"  : "0.1",
    "author"   : "InfoSaône",
    "category" : "InfoSaône",
    "description": """
Module Odoo 16 pour Plastigray pour migrer les développements de Dynacase dans Odoo
===================================================
""",
    "maintainer" : "InfoSaône",
    "website"    : "http://www.infosaone.com",
    "depends"    : [
        "web",
        "mail",
        "utm",
        "is_plastigray16",
        "is_background_color_widget",
        "is_vllm2odoo",
    ],
    "data" : [
        "security/res.groups.xml",
        "security/ir.rule.xml",
        "security/ir.model.access.csv",
        "security/ir.model.access.xml",
        "views/is_param_project_view.xml",
        "views/is_doc_moule_view.xml",
        "views/is_revue_de_contrat_view.xml",
        "views/is_revue_lancement_view.xml",
        "views/is_dossier_modif_variante_view.xml",
        "views/is_revue_projet_jalon_view.xml",
        "views/is_revue_risque_view.xml",
        "views/is_dossier_appel_offre_view.xml",
        "views/is_fermeture_gantt_view.xml",
        "views/is_mold_project_view.xml",
        "views/is_gantt_pdf_view.xml",
        "views/is_gantt_copie_view.xml",
        "views/is_modele_bilan_view.xml",
        "views/is_mold_view.xml",
        "views/is_mold_maintenance_preventive_view.xml",
        "views/is_dossierf_view.xml",
        "views/is_creation_doc_migration_view.xml",
        "views/is_demande_essai_view.xml",
        "views/is_dossier_article_view.xml",
        "views/is_indicateur_revue_jalon_view.xml",
        "views/is_fiche_codification_view.xml",
        "views/is_fiche_information_prospect_view.xml",
        "views/is_plan_action_view.xml",
        "views/is_plan_amelioration_continu_view.xml",
        "views/is_modif_donnee_technique_view.xml",
        "views/is_prise_avance_view.xml",
        "views/is_demande_modif_compte_fournisseur_view.xml",
        "views/is_demande_modif_tarif_fournisseur_view.xml",
        "views/is_inv_achat_moule_view.xml",
        "views/is_erd_view.xml",
        "views/is_liste_diffusion_mail_view.xml",
        "views/is_facture_outillage_view.xml",
        "views/is_norme_cdc_view.xml",
        "views/is_fnc_view.xml",
        "views/is_reception_view.xml",
        "views/is_factures_view.xml",
        "views/is_reclamation_fournisseur_view.xml",
        "views/is_releve_dimensionnel_view.xml",
        "views/is_ctrl_rcp_view.xml",
        "views/is_processus_view.xml",
        "views/is_planification_audit_view.xml",
        "views/is_res_users_partner_view.xml",
        "views/is_prodstar_view.xml",
        "views/is_oberthur_view.xml",
        "views/is_demande_consultation_view.xml",
        "data/is_oberthur_combinaison_data.xml",
        "wizard/is_processus_doc_revision_wizard_view.xml",
        "wizard/is_revue_projet_jalon_refus_wizard_view.xml",
        "wizard/is_oberthur_etiquette_code_barre_wizard_view.xml",
        "wizard/is_oberthur_recherche_montage_wizard_view.xml",
        "views/report_is_oberthur_liste_saisies.xml",
        "views/report_is_oberthur_etiquette_code_barre.xml",
        "views/report_is_inv_achat_moule.xml",
        "views/report_is_facture_outillage.xml",
        "views/report_is_demande_essai.xml",
        "views/report_is_demande_essai_eti.xml",
        "views/report_is_fnc.xml",
        "views/report_is_reclamation_fournisseur.xml",
        "views/report_is_revue_de_contrat.xml",
        "views/report_is_fiche_codification.xml",
        "views/report_is_modif_donnee_technique.xml",
        "views/report_is_mold_maintenance_preventive_historique.xml",
        "views/is_prompt_ia_view.xml",
        "views/menu.xml",
    ], 
    "qweb": [
    ],
    "assets": {
        'web.assets_backend': [
            #'is_dynacase2odoo/static/src/js/custom_one2many_widget.js',

            'is_dynacase2odoo/static/src/js/many2many_tags_contact.js',
            'is_dynacase2odoo/static/src/css/*',
            'is_dynacase2odoo/static/lib/dhtmlxGantt/*',
            'is_dynacase2odoo/static/src/dhtmlxgantt_project/*',
            'is_dynacase2odoo/static/src/background_color/*',
            'is_dynacase2odoo/static/src/mold_maintenance_pager/*',
            'is_dynacase2odoo/static/src/suivi_projet/*',
            'is_dynacase2odoo/static/src/cartographie_processus/cartographie_processus.css',
            'is_dynacase2odoo/static/src/cartographie_processus/cartographie_processus.js',
            'is_dynacase2odoo/static/src/cartographie_processus/cartographie_processus.xml',

            # 'is_dynacase2odoo/static/src/js/*',
            # 'is_dynacase2odoo/static/src/dynamic_delete_o2m/*',
            # 'is_dynacase2odoo/static/src/dynamic_one2many_delete/*',
            #'is_dynacase2odoo/static/src/js/list_renderer_patch.js',
            #'is_dynacase2odoo/static/src/js/no_form_in_tree.js',


         ],

        'web.report_assets_common': [
            'is_dynacase2odoo/static/src/scss/report.scss',

        ]


    },
    "installable": True,
    "application": True,
    "license": "LGPL-3",
}

